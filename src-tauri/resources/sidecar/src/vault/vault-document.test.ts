...
describe("vault-document", () => {
  test("adds date frontmatter when saving a note without frontmatter", () => {
    ...
    updateVaultDocument(notesPath, relativePath, { body: "Some text\n\nMore text" });
  ...
  test("preserves existing frontmatter date on save", () => {
    const notesPath = makeNotesDir();
    mkdirSync(join(notesPath, "Inbox"), { recursive: true });
    ...
    updateVaultDocument(notesPath, relativePath, { body: "Updated body" });
