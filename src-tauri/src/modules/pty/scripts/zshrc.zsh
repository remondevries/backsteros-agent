# backsteros-shell-integration (zshrc)
#
...
# zsh, so we shadow $? into `_backsteros_ret`.

{
  _backsteros_user_zdotdir="${BACKSTEROS_USER_ZDOTDIR:-$HOME}"
  [ -f "$_backsteros_user_zdotdir/.zshrc" ] && source "$_backsteros_user_zdotdir/.zshrc"
...
if [[ -z "$__BACKSTEROS_HOOKS_LOADED" ]]; then
  __BACKSTEROS_HOOKS_LOADED=1
  ...
  _backsteros_precmd() {
    local _backsteros_ret=$?
    printf '\e]133;D;%s\e\\' "$_backsteros_ret"
    printf '\e]7;file://%s%s\e\\' "${HOST}" "$(_backsteros_urlencode "$PWD")"
    ...
    if [[ -n "$BACKSTEROS_BLOCKS" ]]; then
      ...
      if [[ -n "$_backsteros_block_seen" ]]; then
        PS1=$'\n\n%{\e]133;B\e\\%}'
      else
        PS1=$'\n%{\e]133;B\e\\%}'
      fi
  ...
  _backsteros_preexec() {
    ...
    [[ -n "$BACKSTEROS_BLOCKS" ]] && _backsteros_block_seen=1
    local cmd="${1//[[:cntrl:]]/ }"
    printf '\e]133;C;%s\e\\' "${cmd[1,256]}"
  }

  if (( $+functions[add-zsh-hook] )); then
    add-zsh-hook precmd _backsteros_precmd
    add-zsh-hook preexec _backsteros_preexec
  fi
