...
pub fn build_command(cwd: Option<String>, blocks: bool) -> Result<CommandBuilder, String> {
    #[cfg(unix)]
    {
...
fn apply_common(cmd: &mut CommandBuilder, cwd: Option<String>, blocks: bool) {
    ...
    cmd.env("BACKSTEROS_TERMINAL", "1");
    if blocks {
        cmd.env("BACKSTEROS_BLOCKS", "1");
    }
...
mod unix {
                    ...
                    Ok(zdotdir) => {
                        if let Ok(user_zd) = std::env::var("ZDOTDIR") {
                            if Path::new(&user_zd) != zdotdir.as_path() {
                                cmd.env("BACKSTEROS_USER_ZDOTDIR", user_zd);
                            }
                        }
    ...
    fn integration_root() -> Result<PathBuf, String> {
        ...
        let root = home.join(".cache").join("backsteros").join("shell-integration");
    ...
    fn prepare_fish_conf_d() -> Result<(), String> {
        ...
        fs::create_dir_all(&dir).map_err(|e| format!("create {}: {e}", dir.display()))?;
        write_if_changed(&dir.join("backsteros.fish"), FISH_INIT)?;
        Ok(())
    }
