...
import type { DocumentStatusGroup } from "../../lib/documentStatusGroups";

function InboxIcon() {
  return (
    <svg className="document-status-icon" viewBox="0 0 16 16" width="14" height="14" aria-hidden="true">
      <path
        fillRule="evenodd"
        clipRule="evenodd"
...
export function DocumentStatusIcon({
  ...
  if (status === "In Progress") {
    return <LinearStatusIcon status="In Progress" stateType="started" title={title ?? status} />;
  }

  if (status === "On Hold") {
    return <LinearStatusIcon status="On Hold" stateType="started" title={title ?? status} />;
  }

  return <LinearStatusIcon status="Done" stateType="completed" title={title ?? status} />;
}
